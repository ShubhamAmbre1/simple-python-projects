...
def button_sub():
    ...
    sub = e.get()
    e.delete(0, END)
    #e.insert(0, sum(add_list) - int(sub))
    add_list.append(sum(add_list) - int(sub))
    print(add_list)
...
but_add = Button(root, text = "+", command = lambda: button_add(), padx = 20, pady = 20)
but_sub = Button(root, text = "-", command = lambda: button_sub(), padx = 20, pady = 20)
but_clear = Button(root, text = "CLEAR", command = button_clear, padx = 6, pady = 20)
but_equal = Button(root, text = "=", command = equal, padx = 46, pady = 20)
...
but_clear.grid(row = 6, column = 1)
but_equal.grid(row = 6,column = 2, columnspan = 2)
